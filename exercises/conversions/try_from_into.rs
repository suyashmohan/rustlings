...
fn isValidColorValue(num: i16) -> bool {
    if num >= 0 && num <= 255 {
        return true;
    }
    false
...
fn isValidRGB(r: i16, g: i16, b:i16) -> bool {
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if isValidRGB(tuple.0, tuple.1, tuple.2) {
            return Ok(Color{
                red: tuple.0 as u8,
                green: tuple.1 as u8,
                blue: tuple.2 as u8
        ...
        Err(String::from("Invalid RGB Values"))
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if isValidRGB(arr[0], arr[1], arr[2]) {
            return Ok(Color{
                red: arr[0] as u8,
                green: arr[1] as u8,
                blue: arr[2] as u8
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() == 3 && isValidRGB(slice[0], slice[1], slice[2]) {
            return Ok(Color{
                red: slice[0] as u8,
                green: slice[1] as u8,
                blue: slice[2] as u8
